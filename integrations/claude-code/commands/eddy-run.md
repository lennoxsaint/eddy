...
1. Call `eddy_edit_start` with `source` set to the footage path in $ARGUMENTS. If the user gave a focus brief, pass it as `focus`. Capture the returned `job_id` and `run_dir`.
...
4. On `failed`: surface the `log_tail` from the status payload and inspect the run folder for `one-sentence-state.json` or `support-bundle.zip` so the user can see the exact blocker.
