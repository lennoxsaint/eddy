...
7. As a creator, I want the loop capped at 5 iterations with best-attempt shipping and receipts, so that a run always terminates with the strongest result it found.
...
29. As a new external user, I want to install Eddy with pipx and be editing within minutes via a stranger-readable quickstart, so that the app is shippable to a thousand users, not just its author.
...
31. As the product owner, I want the whole build tracked as tiny Linear issues updated live, so that progress is visible on the board at all times.
...
- **Done gate:** all deterministic gates pass AND judge ≥ 8/10, max 5 iterations, then best attempt ranked by (gates passed, judge score, distance to duration target), shipped with `shipped_with_failures` receipts when applicable.
...
- **Shorts render ports the proven standard** (Yasmine's "Clients Hunt You v3", vendored): 1080×1920 stacked layout, face panel, karaoke caption zone (3–6 word groups, blue current-word highlight, spoken/future word dimming), screen panel, navy background; audio-safe handles 0.24s start / 0.32s internal / 0.52s final; sentence-final QA ledger per short. Degraded single-composite layout (no screen track) is the primary path; dual-source is the rich case.
...
- **Hard gates enforced in code:** sources opened read-only and hash-verified before/after every run; every ffmpeg output path asserted inside the run directory; no publish/upload integration exists in v1; repo stays local until explicit release approval.
