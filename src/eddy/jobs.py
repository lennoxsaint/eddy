"""Subprocess job manager — shared by the MCP server and the TUI.
...
is robust even across a restart (the run dir is the source of truth). The job id is the run slug for
single-run kinds, which makes the run dir deterministic (``runs_dir/<slug>``).

Lives in core `eddy` (no MCP-SDK deps) so the TUI can launch/monitor runs without the optional `mcp`
extra; `eddy.mcp_server.jobs` re-exports it for back-compatibility. `spawn` is injectable so tests
exercise the manager without launching real processes.
"""
